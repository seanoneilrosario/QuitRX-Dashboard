...
export default function BundleCreateFields({ products, variants, bundleProductIds, brands, productTypes, initialName = "", initialBrandId = "", initialProductTypeId = "" }: {
  initialName?: string;
  ...
  return <section className={styles.formCard}>
    ...
    <div className={styles.formGrid}>
      ...
      <label>Vendor<select name="brandId" required defaultValue={initialBrandId}>
        <option value="" disabled>Select a vendor</option>
        {brands.map((brand) => <option key={brand.id} value={brand.id}>{brand.label}</option>)}
      </select></label>
      <label>Type<select name="productTypeId" required defaultValue={initialProductTypeId}>
