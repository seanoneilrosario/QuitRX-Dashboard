...
function metafieldValue(item: RetailRecord, key: string) {
  if (Array.isArray(item.metafields)) return (item.metafields as RetailRecord[]).find((field) => field.key === key)?.value;
  const metafields = nested(item, "metafields");
  const value = metafields?.[key];
  return value && typeof value === "object" ? (value as RetailRecord).value : value;
}
function customerTags(item: RetailRecord) {
  ...
  return item.tags.flatMap((tag) => {
    ...
    const label = text(linkedTag?.name ?? record.name ?? record.label ?? record.value, "").trim();
    return label ? [label] : [];
  });
}
...
function Customers({ items, query, page, pagination, error }: { items: RetailRecord[]; query: string; page: number; pagination: RetailPagination; error?: string }) {
  const filtered = items.filter((item) => !query || `${text(item.firstName)} ${text(item.lastName)} ${text(item.email)} ${text(item.phone, "")} ${customerTags(item).join(" ")}`.toLowerCase().includes(query.toLowerCase()));
  const filteredPagination = query ? { page, limit: 50, total: filtered.length, totalPages: Math.max(1, Math.ceil(filtered.length / 50)) } : pagination;
  const visibleItems = query ? filtered.slice((page - 1) * 50, page * 50) : filtered;
  return <><Header title="Customers" description="Search customer accounts, purchase history and prescription status."/><Notice message={error}/><div className={styles.toolbar}><Search placeholder="Search name, email, phone or tag" query={query}/></div><Table heads={["Customer", "Contact", "Tags", "Orders", "Total spent", "Status", ""]}>{visibleItems.map((item, index) => { const tags = customerTags(item); return <tr key={text(item.id, String(index))}><td><strong>{text(item.firstName)} {text(item.lastName, "")}</strong><small>{text(item.id)}</small></td><td>{text(item.email)}<small>{text(item.phone)}</small></td><td><div className={styles.organizationChips}>{tags.length ? tags.map((tag) => <span className={styles.tagChip} key={tag}>{tag}</span>) : <small>No tags</small>}</div></td><td>{text(item.numberOfOrders, "0")}</td><td>{money(item.totalSpent)}</td><td><Status value={item.state}/></td><td><Link href={`/dashboard/customers/details?id=${text(item.id)}`}>View</Link></td></tr>; })}</Table><Pagination pagination={filteredPagination} path="/dashboard/customers" query={query}/></>;
}

function CustomerDetail({ item, editing, error }: { item?: RetailRecord; editing?: boolean; error?: string }) {
  if (!item) return <><Header title="Customer not found" description="Choose a customer from the customer list."/><Notice message={error}/><Link className={styles.primary} href="/dashboard/customers">Back to customers</Link></>;
  if (editing) {
    const meta = (field: string, key: string) => metafieldValue(item, key) ?? item[field];
    const tags = customerTags(item);
    return <><Header title="Edit customer" description="Update customer information, metafields and tags in QuitHero."/><Notice message={error}/><form action={saveResource} className={styles.form}><input type="hidden" name="_resource" value="customers"/><input type="hidden" name="_id" value={text(item.id)}/><input type="hidden" name="_returnTo" value={`/dashboard/customers/details?id=${text(item.id)}`}/><section className={styles.formCard}><h2>Customer details</h2><div className={styles.formGrid}>{[["firstName","First name"],["lastName","Last name"],["email","Email"],["phone","Phone"]].map(([name,label]) => <label key={name}>{label}<input name={name} defaultValue={text(item[name], "")}/></label>)}</div></section><section className={styles.formCard}><h2>Tags</h2><TagsInput initialTags={tags}/></section><section className={styles.formCard}><h2>Metafield</h2><div className={styles.formGrid}><label>Script expiry<input type="date" name="scriptExpiry" defaultValue={dateInput(meta("scriptExpiry", "script_expiry"))}/></label><label>Script ID<input name="scriptId" defaultValue={text(meta("scriptId", "script_id"), "")}/></label><label>Consult purchase<select name="consultPurchase" defaultValue={String(meta("consultPurchase", "consult_purchase") ?? false)}><option value="true">True</option><option value="false">False</option></select></label><label>Birthday<input type="date" name="birthday" defaultValue={dateInput(meta("birthday", "birthday"))}/></label><label>Script validity<input name="scriptValidity" defaultValue={text(meta("scriptValidity", "script_validity"), "")}/></label><label>Renewal form<input name="renewalForm" defaultValue={text(meta("renewalForm", "renewal_form"), "")}/></label><label>Script active<select name="scriptActive" defaultValue={String(meta("scriptActive", "script_active") ?? false)}><option value="true">True</option><option value="false">False</option></select></label><label>Gender<input name="gender" defaultValue={text(meta("gender", "gender"), "")}/></label><label>Vape tag<input name="vapeTag" defaultValue={text(meta("vapeTag", "vape_tag"), "")}/></label><label>Pouch tag<input name="pouchTag" defaultValue={text(meta("pouchTag", "pouch_tag"), "")}/></label><label className={styles.full}>Document<input name="document" defaultValue={text(meta("document", "document"), "")}/></label><label>Social login<input name="socLogin" defaultValue={text(meta("socLogin", "soc_login"), "")}/></label><label>Script uploaded<input name="scriptUploaded" defaultValue={text(meta("scriptUploaded", "script_uploaded"), "")}/></label></div></section><div className={styles.formActions}><Link href={`/dashboard/customers/details?id=${text(item.id)}`}>Cancel</Link><button className={styles.primary}>Save changes</button></div></form></>;
  }
  const tags = customerTags(item);
  return <><Header title={`${text(item.firstName)} ${text(item.lastName, "")}`} description={text(item.email)} action={<Link className={styles.primary} href={`/dashboard/customers/edit?id=${text(item.id)}`}>Edit customer</Link>}/><Notice message={error}/><div className={styles.detailGrid}><section className={styles.card}><h2>Customer information</h2>{[["Email",item.email],["Phone",item.phone],["Birthday",item.birthday],["Account state",item.state],["Verified email",item.verifiedEmail ? "Yes" : "No"]].map(([label,value]) => <div className={styles.detailRow} key={String(label)}><span>{text(label)}</span><strong>{text(value)}</strong></div>)}</section><section className={styles.card}><h2>Prescription & purchases</h2>{[["Total orders",item.numberOfOrders],["Total spent",money(item.totalSpent)],["Script ID",item.scriptId],["Script expiry",item.scriptExpiry],["Script active",item.scriptActive ? "Yes" : "No"]].map(([label,value]) => <div className={styles.detailRow} key={String(label)}><span>{text(label)}</span><strong>{text(value)}</strong></div>)}</section><section className={styles.card}><h2>Tags</h2><div className={styles.organizationChips}>{tags.length ? tags.map((tag) => <span className={styles.tagChip} key={tag}>{tag}</span>) : <small>No tags</small>}</div></section></div></>;
}
...
function Inventory({ variants, history, query, stockFilter, sort, error }: { variants: RetailRecord[]; history?: RetailRecord[]; query: string; stockFilter: string; sort: string; error?: string }) {
  ...
  return <><Header title="Inventory" description="Manage available, allocated and incoming stock by product variant." action={<Link className={styles.secondary} href="/dashboard/inventory/history">View history</Link>}/><Notice message={error}/><div className={styles.toolbar}><form className={`${styles.search} ${styles.inventorySearch}`}><span>⌕</span><input name="q" aria-label="Search inventory" placeholder="Search product, SKU or product ID" defaultValue={query}/><select name="stock" aria-label="Filter by stock" defaultValue={stockFilter}><option value="">All stock</option><option value="in">In stock</option><option value="low">Low stock</option><option value="out">Out of stock</option></select><select name="sort" aria-label="Sort inventory" defaultValue={sort}><option value="product-asc">Product: A–Z</option><option value="product-desc">Product: Z–A</option><option value="sku-asc">SKU: A–Z</option><option value="sku-desc">SKU: Z–A</option><option value="available-desc">Available: high to low</option><option value="available-asc">Available: low to high</option><option value="allocated-desc">Allocated: high to low</option><option value="incoming-desc">Incoming: high to low</option></select><button>Apply</button></form></div><Table heads={["Variant", "SKU", "Available", "Allocated", "Incoming", "Health"]}>{filtered.map((item,index) => <tr key={text(item.id,String(index))}><td><strong>{text(item.name)}</strong><small>{text(item.productId)}</small></td><td>{text(item.sku)}</td><td>{text(item.inventory,"0")}</td><td>{text(item.allocatedInventory,"0")}</td><td>{text(item.incomingInventory,"0")}</td><td><Status value={Number(item.inventory ?? 0) <= 0 ? "OUT OF STOCK" : Number(item.inventory ?? 0) <= 10 ? "LOW STOCK" : "HEALTHY"}/></td></tr>)}</Table>{!filtered.length ? <div className={styles.notice}><strong>No inventory found</strong><span>Try changing the search, stock filter, or sort option.</span></div> : null}</>;
}
...
export default async function DashboardPage({ params, searchParams }: Props) {
  ...
  else if (area === "customers" && !sub) { const result = q ? await safeRetailAll("/customers") : await safeRetailPage("/customers", page, 50); content = <Customers items={result.data} query={q} page={page} pagination={result.pagination} error={result.error}/>; }
  else if (area === "customers") { const result = await safeRetailRecord(`/customers/${encodeURIComponent(id)}`); content = <CustomerDetail item={result.data} editing={sub === "edit"} error={result.error}/>; }
