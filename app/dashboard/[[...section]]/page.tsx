...
import { getFrequentlyBoughtTogether } from "@/lib/sanity-storefront";
...
function ProductForm({ item, brands, productTypes, collections, products = [], recommendationIds = [], recommendationError }: { item?: RetailRecord; brands: RetailRecord[]; productTypes: RetailRecord[]; collections: RetailRecord[]; products?: RetailRecord[]; recommendationIds?: string[]; recommendationError?: string }) {
...
function CollectionEdit({ item, products, error }: { item?: RetailRecord; products: RetailRecord[]; error?: string }) {
  ...
  return <><Header title={`Edit ${text(item.name, "collection")}`} description="Update collection details and product membership."/><Notice message={error}/><section className={styles.formCard}><h2>Collection details</h2><CollectionCreateForm products={collectionProductOptions(products)} initial={item}/></section></>;
}
...
export default async function DashboardPage({ params, searchParams }: Props) {
  ...
  if (area === "dashboard") { const [p,c,o,v] = await Promise.all([safeRetailPage("/products",1),safeRetailPage("/customers",1),safeRetailList("/orders"),safeRetailList("/product-variants")]); content = <Dashboard productTotal={p.pagination.total} customerTotal={c.pagination.total} orders={o.data} variants={v.data} error={p.error ?? c.error ?? o.error ?? v.error}/>; }
    ...
    const [result, brands, productTypes, collections, products, recommendations] = await Promise.all([
      safeRetailRecord(`/products/${encodeURIComponent(id)}`), safeRetailList("/brands"), safeRetailList("/product-type"), safeRetailList("/collections"), safeRetailAll("/products"),
    ...
    content = <ProductForm item={result.data} brands={brands.data} productTypes={productTypes.data} collections={collections.data} products={products.data} recommendationIds={recommendations.data} recommendationError={products.error ?? recommendations.error}/>;
  }
