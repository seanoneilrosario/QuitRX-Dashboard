...
import CollectionCreateFields, { type CollectionProduct, type CollectionTag } from "./collection-create-fields";
...
export default async function DashboardPage({ params, searchParams }: Props) {
  ...
  if (area === "dashboard") { const [p,c,o,v] = await Promise.all([safeRetailPage("/products",1),safeRetailPage("/customers",1),safeRetailList("/orders"),safeRetailList("/product-variants")]); content = <Dashboard productTotal={p.pagination.total} customerTotal={c.pagination.total} orders={o.data} variants={v.data} error={p.error ?? c.error ?? o.error ?? v.error}/>; }
    ...
    const [result, productResult, tagResult] = await Promise.all([safeRetailList("/collections"), safeRetailAll("/products"), safeRetailAll("/tags")]);
    const products = productResult.data.flatMap((product) => typeof product.id === "string" ? [{ id: product.id, label: text(product.name, product.id), tags: Array.isArray(product.tags) ? product.tags.flatMap((tag) => typeof tag === "string" ? [tag] : tag && typeof tag === "object" ? [text((tag as RetailRecord).id, ""), text((tag as RetailRecord).name, "")].filter(Boolean) : []) : [] }] : []);
    const tags = tagResult.data.flatMap((tag) => typeof tag.id === "string" ? [{ id: tag.id, label: text(tag.name, tag.id) }] : []);
    content = <ResourcePage kind="collections" items={result.data} products={products} tags={tags} error={result.error ?? productResult.error ?? tagResult.error} path="/dashboard/collections"/>;
  }
